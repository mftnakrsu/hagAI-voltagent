  ...
  const getWeeklyPlannedTasks = createTool({
    ...
    execute: async ({ assignee, excludeCompleted }) => {
      try {
        ...
        if (assignee) {
          tasks = await asanaClient.getTasks({
            assignee,
          });
          // Filter locally
          tasks = tasks.filter(dateFilter);
        } else {
          // No assignee -> Fetch workspace wide
          tasks = await asanaClient.getAllAssignedTasks(dateFilter);
        }
  ...
  const getTeamWorkload = createTool({
    ...
    execute: async ({ includeCompleted }) => {
      try {
        ...
        // Get all tasks
        // Use getAllAssignedTasks to get a full picture of the team's workload
  ...
  const getOverdueTasksByPerson = createTool({
    ...
    execute: async () => {
      try {
        ...
        // Use getAllAssignedTasks for better team coverage
        const tasks = await asanaClient.getAllAssignedTasks();
