...
export class AsanaClient {
  ...
  /**
   * Get all tasks assigned to all users in the workspace
   * This is useful for "team workload" or "all planned tasks" queries
   * Iterates through all users to fetch their tasks
   */
  async getAllAssignedTasks(
    filter?: (task: AsanaTask) => boolean,
    completedSince?: string
  ): Promise<AsanaTask[]> {
    const users = await this.getWorkspaceUsers();
    const allTasks = new Map<string, AsanaTask>();
    ...
    for (let i = 0; i < users.length; i += chunkSize) {
      ...
      const promises = chunk.map(user =>
        this.getTasks({
          ...
          console.warn(`Failed to fetch tasks for user ${user.name}:`, err);
          return [] as AsanaTask[];
        })
      );
   ...
   * useful for finding unassigned tasks
   */
    ...
    for (let i = 0; i < projects.length; i += chunkSize) {
      ...
      const promises = chunk.map(project =>
        this.getTasks({
          ...
          console.warn(`Failed to fetch tasks for project ${project.name}:`, err);
          return [] as AsanaTask[];
        })
      );
